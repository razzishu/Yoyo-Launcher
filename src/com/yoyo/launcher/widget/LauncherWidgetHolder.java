...
import static com.yoyo.launcher.widget.LauncherAppWidgetProviderInfo.fromProviderInfo;
import static com.yoyo.launcher.widget.ListenableAppWidgetHost.getWidgetHolderExecutor;
...
import com.yoyo.launcher.widget.ListenableAppWidgetHost.ProviderChangedListener;
...
public class LauncherWidgetHolder {
    ...
    public void startListening() {
        ...
        getWidgetHolderExecutor().execute(() -> {
            ...
            // Standalone fix: also schedule a delayed update to handle race conditions
            MAIN_EXECUTOR.getHandler().postDelayed(this::updateDeferredView, 1000);
        });
    }
    ...
    /**
     * Returns an {@link ActivityOptions} bundle from the {code activity} for launching
    ...
    protected LauncherAppWidgetHostView createViewInternal(
            int appWidgetId, @NonNull LauncherAppWidgetProviderInfo appWidget) {
        if ((mFlags.get() & FLAG_LISTENING) == 0) {
            Log.d(TAG, "createViewInternal: Not listening, returning PendingAppWidgetHostView for id=" + appWidgetId);
            ...
            if (Looper.myLooper() != Looper.getMainLooper()) {
                Log.d(TAG, "createViewInternal: Not on main thread, returning ListenableHostView for id=" + appWidgetId);
            ...
            try {
                Log.d(TAG, "createViewInternal: Calling mWidgetHost.createView for id=" + appWidgetId);
                LauncherAppWidgetHostView view = (LauncherAppWidgetHostView) mWidgetHost.createView(
                        mContext, appWidgetId, appWidget);
                if (view == null) {
                    Log.e(TAG, "createViewInternal: mWidgetHost.createView returned null for id=" + appWidgetId);
                    view = new ListenableHostView(mContext);
                    view.setAppWidget(appWidgetId, appWidget);
                }
                return view;
            } catch (Exception e) {
                Log.e(TAG, "createViewInternal: Error creating widget view for id=" + appWidgetId, e);
