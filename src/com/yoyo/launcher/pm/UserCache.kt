...
import com.yoyo.launcher.Utilities.ATLEAST_U
import com.yoyo.launcher.Utilities.ATLEAST_V
...
import com.yoyo.launcher.icons.UserBadgeDrawable
    ...
    private fun rebuildUserCache(): UserManagerState =
        UserManagerState(
                fetchSafe(emptyList<UserHandle>()) { userProfiles }
                    .also { Log.d(TAG, "rebuildUserCache: found ${it.size} profiles") }
                    .mapNotNull { buildCachedUserInfo(it) }
                    .associateBy { it.iconInfo.user }
            )
            .also {
                _userInfoMap = it
                Log.d(TAG, "rebuildUserCache: cache rebuilt with ${it.userProfiles.size} users")
            }

    private fun buildCachedUserInfo(user: UserHandle): CachedUserInfo? {
        Log.d(TAG, "buildCachedUserInfo: for user $user")
        if (!ATLEAST_V) {
        ...
        Log.d(TAG, "buildCachedUserInfo: getLauncherUserInfo returned $userInfo")
