...
import static com.yoyo.launcher.accessibility.LauncherAccessibilityDelegate.DISMISS_PREDICTION;
import static com.yoyo.launcher.accessibility.LauncherAccessibilityDelegate.INVALID;
import static com.yoyo.launcher.accessibility.LauncherAccessibilityDelegate.RECONFIGURE;
import static com.yoyo.launcher.accessibility.LauncherAccessibilityDelegate.UNINSTALL;
import static com.yoyo.launcher.logging.StatsLogManager.LauncherEvent.LAUNCHER_ITEM_DROPPED_ON_DONT_SUGGEST;
import static com.yoyo.launcher.logging.StatsLogManager.LauncherEvent.LAUNCHER_ITEM_DROPPED_ON_UNINSTALL;
import static com.yoyo.launcher.logging.StatsLogManager.LauncherEvent.LAUNCHER_ITEM_UNINSTALL_CANCELLED;
import static com.yoyo.launcher.logging.StatsLogManager.LauncherEvent.LAUNCHER_ITEM_UNINSTALL_COMPLETED;
import static com.yoyo.launcher.model.data.ItemInfoWithIcon.FLAG_SYSTEM_MASK;
import static com.yoyo.launcher.model.data.ItemInfoWithIcon.FLAG_SYSTEM_NO;
...
public class SecondaryDropTarget extends ButtonDropTarget implements OnAlarmListener {
    ...
    private int getButtonType(ItemInfo info, View view) {
        ...
        if (com.yoyo.launcher.Flags.enablePrivateSpace() && UserCache.getInstance(getContext()).getUserInfo(
                info.user).isPrivate()) {
            return INVALID;
        }
    ...
    public static ComponentName getUninstallTarget(Context context, ItemInfo item) {
        ...
        if (cn != null && (item.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION
                || item.itemType == LauncherSettings.Favorites.ITEM_TYPE_DEEP_SHORTCUT)) {
            String packageName = cn.getPackageName();
            try {
                LauncherApps la = context.getSystemService(LauncherApps.class);
                ApplicationInfo appInfo = la.getApplicationInfo(packageName, 0, user);
                if (appInfo != null) {
                ...
                // Fallback to resolveActivity
                LauncherActivityInfo info = context.getSystemService(LauncherApps.class)
                        .resolveActivity(item.getIntent(), user);
                if (info != null) {
                    boolean isSystem = (info.getApplicationInfo().flags & ApplicationInfo.FLAG_SYSTEM) != 0;
                    if (!isSystem) {
    ...
    public static ComponentName performUninstall(Context context, @Nullable ComponentName cn,
        ...
        String packageName = cn.getPackageName();
        Log.d(TAG, "performUninstall: triggering for " + packageName + " user=" + info.user);
        try {
            Intent i = new Intent(Intent.ACTION_DELETE, Uri.fromParts("package", packageName, null))
                    .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            
            if (info.user != null && !info.user.equals(Process.myUserHandle())) {
                i.putExtra(Intent.EXTRA_USER, info.user);
            }
            context.startActivity(i);
            return cn;
        } catch (Exception e) {
            Log.e(TAG, "performUninstall failed", e);
            return null;
        }
    }
    ...
    protected class DeferredOnComplete implements DragSource {
        ...
        public void onLauncherResume() {
            Log.d(TAG, "onLauncherResume: pkg=" + mPackageName);
