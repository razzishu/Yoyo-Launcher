...
import static com.yoyo.launcher.InvariantDeviceProfile.INDEX_DEFAULT;
import static com.yoyo.launcher.InvariantDeviceProfile.INDEX_LANDSCAPE;
import static com.yoyo.launcher.InvariantDeviceProfile.INDEX_TWO_PANEL_LANDSCAPE;
import static com.yoyo.launcher.InvariantDeviceProfile.INDEX_TWO_PANEL_PORTRAIT;
import static com.yoyo.launcher.InvariantDeviceProfile.createDisplayOptionSpec;
import static com.yoyo.launcher.Utilities.dpiFromPx;
import static com.yoyo.launcher.deviceprofile.DevicePropertiesKt.createWindowBounds;
import static com.yoyo.launcher.folder.ClippedFolderIconLayoutRule.ICON_OVERLAP_FACTOR;
import static com.yoyo.launcher.icons.IconNormalizer.ICON_VISIBLE_AREA_FACTOR;
import static com.yoyo.launcher.taskbar.TaskbarManagerImpl.ENABLE_TASKBAR_URI;
import static com.yoyo.launcher.testing.shared.ResourceUtils.INVALID_RESOURCE_HANDLE;
import static com.yoyo.launcher.testing.shared.ResourceUtils.pxFromDp;
import static com.yoyo.launcher.util.OverviewReleaseFlags.enableGridOnlyOverview;
...
import com.yoyo.launcher.CellLayout.ContainerType;
import com.yoyo.launcher.InvariantDeviceProfile.DisplayOptionSpec;
import com.yoyo.launcher.deviceprofile.AllAppsProfile;
import com.yoyo.launcher.deviceprofile.BottomSheetProfile;
import com.yoyo.launcher.deviceprofile.DeviceProperties;
import com.yoyo.launcher.deviceprofile.DropTargetProfile;
import com.yoyo.launcher.deviceprofile.FolderProfile;
import com.yoyo.launcher.deviceprofile.HotseatProfile;
import com.yoyo.launcher.deviceprofile.OverviewProfile;
import com.yoyo.launcher.deviceprofile.TaskbarProfile;
import com.yoyo.launcher.deviceprofile.WorkspaceProfile;
import com.yoyo.launcher.icons.DotRenderer;
import com.yoyo.launcher.model.data.ItemInfo;
import com.yoyo.launcher.responsive.CalculatedCellSpec;
import com.yoyo.launcher.responsive.CalculatedHotseatSpec;
import com.yoyo.launcher.responsive.CalculatedResponsiveSpec;
import com.yoyo.launcher.responsive.HotseatSpecsProvider;
import com.yoyo.launcher.responsive.ResponsiveCellSpecsProvider;
import com.yoyo.launcher.responsive.ResponsiveSpec.Companion.ResponsiveSpecType;
import com.yoyo.launcher.responsive.ResponsiveSpec.DimensionType;
import com.yoyo.launcher.responsive.ResponsiveSpecsProvider;
import com.yoyo.launcher.util.DisplayController;
import com.yoyo.launcher.util.DisplayController.Info;
import com.yoyo.launcher.util.IconSizeSteps;
import com.yoyo.launcher.util.ResourceHelper;
import com.yoyo.launcher.util.SettingsCache;
import com.yoyo.launcher.util.WindowBounds;
...
public class DeviceProfile {
    ...
    public DeviceProfile() {
        ...
        hotseatProfile = new HotseatProfile(false, 0, 0, 0, 0, 0, 0, false, 0, 0, 0, 0, 0, 0);
    ...
    DeviceProfile(InvariantDeviceProfile inv, Info info,
        ...
        if (mIsResponsiveGrid) {
            ...
            hotseatQsbSpace = inv.enableQsbOnHotseat ? mResponsiveHotseatSpec.getHotseatQsbSpace() : 0;
            ...
            hotseatQsbSpace = inv.enableQsbOnHotseat ? pxFromDp(inv.hotseatQsbSpace[mTypeIndex], mMetrics) : 0;
        ...
        if (!isVerticalBarLayout()) {
            if (inv.enableQsbOnHotseat && mInsets.bottom + minQsbMargin > hotseatBarBottomSpace) {
        ...
        // Force unified icon size everywhere based on universal base size and sizeFactor
        int unifiedIconSizePx = inv.iconBitmapSize;
        mWorkspaceProfile = mWorkspaceProfile.changeIconSize(unifiedIconSizePx);

        updateIconSize(mWorkspaceProfile.getScale(), context);

        // Ensure AllApps uses the same size
        mAllAppsProfile = mAllAppsProfile.copyWithIconSizePx(unifiedIconSizePx);
        updateHotseatSizes(unifiedIconSizePx);
    ...
    private boolean isQsbInline(
        ...
        if (!inv.enableQsbOnHotseat) return false;
    ...
    private int calculateQsbWidth(int hotseatBorderSpace) {
        if (!inv.enableQsbOnHotseat) return 0;
    ...
    public void recalculateHotseatWidthAndBorderSpace() {
        if (!mIsScalableGrid) {
            hotseatQsbWidth = calculateQsbWidth(0);
            return;
        }
