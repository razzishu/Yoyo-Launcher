...
import static com.yoyo.launcher.LauncherSettings.Favorites.CONTAINER;
import static com.yoyo.launcher.LauncherSettings.Favorites.ITEM_TYPE;
import static com.yoyo.launcher.LauncherSettings.Favorites.ITEM_TYPE_APP_PAIR;
import static com.yoyo.launcher.LauncherSettings.Favorites.TABLE_NAME;
import static com.yoyo.launcher.LauncherSettings.Favorites.addTableToDb;
import static com.yoyo.launcher.provider.LauncherDbUtils.tableExists;
...
import com.yoyo.launcher.EncryptionType;
...
import com.yoyo.launcher.backuprestore.LauncherRestoreEventLogger.RestoreError;
...
import com.yoyo.launcher.provider.LauncherDbUtils.SQLiteTransaction;
...
public class ModelDbController {
    ...
    protected DatabaseHelper createDatabaseHelper(boolean forMigration, String dbFile) {
        ...
        if (!tableExists(databaseHelper.getReadableDatabase(), TABLE_NAME)) {
    ...
    /**
     * Migrates the DB. If the migration failed, it retains existing target DB state.
     */
    public void attemptMigrateDb(LauncherRestoreEventLogger restoreEventLogger,
        ...
        // If source and destination grid states are identical, no migration needed
        if (srcDeviceState.isCompatible(destDeviceState)
                && srcDeviceState.getColumns().equals(destDeviceState.getColumns())
                && srcDeviceState.getRows().equals(destDeviceState.getRows())) {
            FileLog.d(TAG, "attemptMigrateDb: source and destination grid states are identical, skipping migration");
            return;
        }
        ...
        try {
            ...
            if (sourceGridMigrationOption != null && destinationGridMigrationOption != null
                ...
                mOpenHelper = createDatabaseHelper(true, destDeviceState.getDbFile());
            ...
            FileLog.e(TAG, "attemptMigrateDb: Failed to migrate grid", e);
            mOpenHelper = createDatabaseHelper(false, destDeviceState.getDbFile());
            destDeviceState.writeToPrefs(mContext);
        } finally {
    ...
    public IntArray deleteEmptyFolders() {
        ...
        try (SQLiteTransaction t = new SQLiteTransaction(db)) {
            ...
            String selection = ITEM_TYPE + " = "
                    + Favorites.ITEM_TYPE_FOLDER + " AND "
                    + _ID +  " NOT IN (SELECT "
                    + CONTAINER + " FROM "
                    + TABLE_NAME + ")";

            IntArray folderIds = LauncherDbUtils.queryIntArray(false, db, TABLE_NAME,
