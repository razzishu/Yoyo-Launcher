...
public class PredictedAppIcon extends DoubleShadowBubbleTextView {
    ...
    public void pin(WorkspaceItemInfo info) {
        if (mIsPinned) return;
        mIsPinned = true;
        // Match workspace icon behavior for labels
        setCenterVertically(mDeviceProfile.getWorkspaceIconProfile().getIconCenterVertically());
        applyFromWorkspaceItem(info);
        // Force text to show immediately
        setText(info.title);
        // Ensure ring is removed visually
        mRingScale = 0f;
        setOnLongClickListener(ItemLongClickListener.INSTANCE_WORKSPACE);
        if (getLayoutParams() instanceof CellLayoutLayoutParams) {
