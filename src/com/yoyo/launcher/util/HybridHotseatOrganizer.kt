...
import com.yoyo.launcher.LauncherAnimUtils
...
import com.yoyo.launcher.anim.AnimatorListeners
...
import com.yoyo.launcher.util.FlagDebugUtils.appendFlag
import com.yoyo.launcher.views.ActivityContext
import com.yoyo.launcher.views.PredictedAppIcon
import com.yoyo.launcher.views.PredictedAppIcon.PredictedIconOutlineDrawing
...
class HybridHotseatOrganizer(
    ...
    var predictedItems = emptyList<ItemInfo>()
        set(value) {
            android.util.Log.d("HybridHotseatOrganizer", "setPredictedItems: size=${value.size}")
            field = value
    ...
    private fun onHotseatHierarchyChanged() {
        if (pauseFlags == 0 && !loadingCheck.invoke()) {
            ...
            // Notify delegate to refresh its pool if it's a real change
            val delegate = LauncherAppState.getInstance(activity as android.content.Context).model.modelDelegate
            if (delegate is com.yoyo.launcher.model.PredictionModelDelegate) {
                delegate.notifyDockChanged()
            }
        }
    }
    ...
    override fun onDragStart(dragObject: DragObject, options: DragOptions?) {
        val dragInfo = dragObject.dragInfo
        if (dragInfo is WorkspaceItemInfo && isPredictedIcon(dragObject.originalView as? View)) {
            // Convert predicted item to a real item on drag start
            dragInfo.container = Favorites.CONTAINER_HOTSEAT
            // Force refresh the icon view to remove the ring and enable reordering via pin()
            (dragObject.originalView as? PredictedAppIcon)?.pin(dragInfo)
            android.util.Log.d("HybridHotseatOrganizer", "Converted predicted item to real item for drag")
        }
