...
package com.yoyo.launcher.graphics
...
import com.yoyo.launcher.LauncherPrefs.Companion.backedUpItem
...
import com.yoyo.launcher.graphics.ShapeDelegate.Companion.DEFAULT_PATH_SIZE_INT
import com.yoyo.launcher.graphics.ShapeDelegate.Companion.pickBestShape
import com.yoyo.launcher.graphics.theme.IconThemeFactory
import com.yoyo.launcher.graphics.theme.ThemePreference
import com.yoyo.launcher.graphics.theme.ThemePreference.Companion.MONO_THEME_VALUE
import com.yoyo.launcher.icons.DotRenderer.IconShapeInfo
import com.yoyo.launcher.icons.GraphicsUtils.generateIconShape
import com.yoyo.launcher.icons.IconShape
import com.yoyo.launcher.icons.IconThemeController
import com.yoyo.launcher.shapes.IconShapeModel.Companion.DEFAULT_ICON_RADIUS
...
import com.yoyo.launcher.util.ListenableRef
...
import com.yoyo.launcher.util.SimpleBroadcastReceiver.Companion.packageFilter
...
constructor(
    ...
    init {
        iconState = parseIconState(null)
        val initialShape = iconShape.createIconShape(DEFAULT_PATH_SIZE_INT)
        _iconShapeData.dispatchValue(initialShape)

        val receiver = SimpleBroadcastReceiver(context, uiExecutor) { refresh() }
        ...
        val prefListener = LauncherPrefChangeListener {
            if (it == PREF_ICON_SHAPE.sharedPrefKey) refresh()
        }
        prefs.addListener(prefListener, PREF_ICON_SHAPE)
        lifecycle.addCloseable(themePreference.forEach(mainExecutor) { refresh() })
    ...
    /** Manually triggers a refresh of the icon state from preferences */
    fun refresh() {
        val newState = parseIconState(iconState)
        Log.d("ThemeManager", "Refreshing icon state: mask=${newState.iconMask}")
        
        // Update global icon shape mask
        IconShape.updateGlobalShape(newState.iconShape.getPath())
    ...
    private fun parseIconState(oldState: IconState?): IconState {
        ...
        android.util.Log.d("ThemeManager", "parseIconState: themeKey=$themeKey factories=${iconThemeFactories.keys}")
        ...
        val iconControllerFactory =
            if (oldState?.themeCode == themeCode) {
                ...
                themeKey?.run { 
                    val factory = iconThemeFactories[factoryId] ?: com.yoyo.launcher.graphics.theme.MonoIconThemeFactory
                    factory.createController(themeId)
                }
            }
    ...
    data class IconState(
        ...
        fun toUniqueId() = "$themeCode,$iconMask,$iconPack"
