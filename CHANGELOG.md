...
- **Live Switch Toggle Updates**: Fixed `RecyclerView.AdapterDataObserver` data forwarding in `SettingsActivity` so switch toggles animate and update state visually in real-time.

### 🖼️ Icon Customization & Drag-and-Drop
...
- **Consistent Custom Icon Pack Dragging**: Resolved custom icon pack morphing during drag-and-drop (`FloatingIconView`, `DragView`, `Utilities.getFullDrawable`) so custom icons stay consistent throughout drag and drop.
- **Expanded Adaptive Foreground**: Scaled adaptive icon foreground to `126%` (`fgScale = 1.26f`) to expand inner app logos and fill custom shape silhouettes cleanly.
